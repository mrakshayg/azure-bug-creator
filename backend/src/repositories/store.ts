...
function mapDraft(record: any): DraftRecord {
  return {
    ...
    linkedUserStoryId: record.linkedUserStoryId ?? '',
    linkedUserStoryMode: (record.linkedUserStoryMode === 'Related' ? 'Related' : 'Child'),
...
export async function updateDraftRecord(userId: string, draftId: string, updates: Partial<DraftRecord>) {
  ...
  const updated = await (await prisma()).bugDraft.update({
    ...
    data: {
      ...
      linkedUserStoryId: updates.linkedUserStoryId,
      linkedUserStoryMode: updates.linkedUserStoryMode,
